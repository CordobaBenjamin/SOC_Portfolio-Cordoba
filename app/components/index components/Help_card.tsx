...
const Help_card = () => {
  ...
  return (
    <>
      <div
        className={`w-4/5 md:w-2/3 xl:w-2/5 xl:mx-6 xl:mb-0 mx-auto px-10 pt-2 pb-10 mb-5 ${overlap}  border-2 ${borderSecondary} rounded-md`}
      >
        <div
        ...
        <span className={`md:text-xl  lg:text-2xl ${textMain}`}>{span1}</span>
        <span
          className={`md:text-xl  lg:text-2xl ${textSecondary} font-semibold`}
        >
          {span2}
        </span>
        <span className={`md:text-xl  lg:text-2xl ${textMain}`}>{span3}</span>
      </div>
    </>
  );
};
